...
public class ScenarioValidator implements Visitor{
    ...
    private String validateScenario(){
        String enumaration = "";
        if(scenarioBody.scenarios.length < 1){
            return "#NoScenario";
        }
        for(int i = 0; i< scenarioBody.scenarios.length-2; i++){
            //add to return the line
            enumaration += scenarioBody.scenarios[i].content;
            enumaration += "\n";
            if(stepStartsWithKeyword(scenarioBody.scenarios[i])){
                if(scenarioBody.scenarios[i].depth != scenarioBody.scenarios[i+1].depth-1){
        ...
        enumaration += scenarioBody.scenarios[scenarioBody.scenarios.length-1].content;
        enumaration += "\n";
        if(stepStartsWithKeyword(scenarioBody.scenarios[scenarioBody.scenarios.length-1])){
            enumaration+= "#KeyWordBroken";
            enumaration += "\n";
        }
        return enumaration;
     ...
     *
     * @return String "True" when scenario Validation passed, "False" when it didn't.
     */
    @Override
